...
import styles from "@/styles/components/render-workspace.module.scss"

export function RenderWorkspace({ content }: ContentType) {
  return (
    <>
      {content.map((value, index) => {
        ...
        if (type === "image") {
          return <RenderImage url={data.file.url} />
        }

        if (type === "attaches") {
          const _split = data.title.split(".")
          const extension = _split[_split.length - 1]!

          const isValidExtension = ["docx", "doc", "pdf"].includes(extension)

          return (
            <div className={styles["file-attaches-wrapper"]}>
              <Link href={data.file.url} target={"_blank"}>
                <div className={styles["file-attaches-container"]}>
                  <div className={styles["file-icon-wrapper"]}>
                    <div
                      className={styles["file-icon-background"]}
                      data-file-type={extension}
                    ></div>
                    <div
                      className={styles["file-icon-name"]}
                      data-file-type={extension}
                    >
                      {isValidExtension ? extension : "400"}
                    </div>
                  </div>

                  <div className={styles["file-attaches-name"]}>
